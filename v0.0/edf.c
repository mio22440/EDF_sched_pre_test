...
void edf_sched(int tick)
{
    ...
    else
    {
        ...
        if(cur_task->period == 4)
        {
            printf("\033[31m %.2d-%s-%d, left:%d, next_arrive:%d \033[0m\n", tick, cur_task->name, cur_task->sched_times, cur_task->left_exec_time, cur_task->next_arrive);
        }
        else
        {
            printf("\033[32m %.2d-%s-%d, left:%d, next_arrive:%d \033[0m\n", tick, cur_task->name, cur_task->sched_times, cur_task->left_exec_time, cur_task->next_arrive);
        }
        // printf("%.2d-%s-%d, left:%d, next_arrive:%d \033[0m\n", tick, cur_task->name, cur_task->sched_times, cur_task->left_exec_time, cur_task->next_arrive);
    }
}
...
edf_task_t *edf_get_highest(int tick)
{
    ...
    while(tmp_task != &edf_rdy_head)
    {
        ...
        else
        {
            if(
                (NULL == ret) ||
                (tmp_task->deadline < ret->deadline)
                )
            {
                ret = tmp_task;
            }
...
void edf_deal_wait_list(void)
{
    ...
    while(tmp_task != &edf_wait_head)
    {
        ...
        if(0 == tmp_task->next_arrive)
        {
            ...
            edf_task_insert_tail(&edf_rdy_head, tmp_task);
